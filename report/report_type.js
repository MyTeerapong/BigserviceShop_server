...
async function generateReportType(req, res) {
  try {
    const [rows] = await pool.execute('SELECT T_id, T_name FROM type');

    const body = [
      ['รหัสประเภทสินค้า', 'ชื่อประเภทสินค้า'],
      ...rows.map(item => [item.T_id, item.T_name]),
    ];
 ...
 const docDefinition = {
  ...
  content: [
    { text: 'รายงานประเภทสินค้า', style: 'header' },
    {
      table: {
        widths: ['auto', '*'],
        body,
  ...
  styles: {
    ...
    pdfDoc.on('end', () => {
      ...
      res.setHeader('Content-Disposition', 'attachment; filename=report_type.pdf');
      res.send(pdfBuffer);
    });
    pdfDoc.end();
